...
| `src/views/settings/LongRunningMinutesInput.test.jsx` | Long-running threshold duration picker (issue #111): splits the stored minutes across native hours/minutes `<select>`s, commits the combined total on either picker change, turns the reminder off at 0h 0m, clamps/normalizes out-of-range and non-finite values |
