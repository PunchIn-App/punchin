...
8. Commit everything in a single commit: `chore: bump to vX.Y.Z` (or fold the bump into the feature PR)
   ...
   > **Merging that PR ships it.** Cloudflare Workers Builds deploys every push to `main`
   > automatically, so users have the code the moment it merges — before step 9 runs. The release
   > below therefore *records* a version that is already live; it does not publish it. That is why
   > step 9 is easy to skip and why skipping it is not harmless: it leaves production running a
   > version with no tag, no notes, and nothing to roll back to. `SECURITY.md` also claims support
   > for a version nobody can download. Do step 9 in the same sitting as the merge.
