...
- **Labor types now have an icon.** Pick a glyph when you create or edit a labor type — it shows up alongside the colour everywhere the type appears (timer, timesheets, analytics legend, invoices), so categories are told apart by shape as well as colour (better for colour-blind users). Existing labor types keep working and get a neutral default icon. ([brand refresh](https://github.com/PunchIn-App/punchin))
