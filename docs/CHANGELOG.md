...
- **Billing profile + invoice band.** A new **Billing** settings section captures your "Billed from" identity (name, business, email, phone, address, payment terms, notes); invoices now print a **Billed from / Billed to** band, and an optional **invoice number** (prefix + counter that advances automatically each time you generate an invoice). ([brand refresh](https://github.com/PunchIn-App/punchin))
