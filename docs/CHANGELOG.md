...
- **Bespoke job & labor pickers everywhere they carry colour/glyph.** The plain dropdowns for picking a job or a labor type are replaced by a custom picker that shows each job's **colour dot + client** and each labor type's **glyph in its colour**, with a check on the current choice — starting with the **Add / Edit time entry** sheet. (The theme, currency, and weekday menus keep the native control.) ([brand refresh](https://github.com/PunchIn-App/punchin))
