...
### Added
- **Billing profile + invoice band.** A new **Billing** settings section captures your "Billed from" identity (name, business, email, phone, address, payment terms, notes); invoices now print a **Billed from / Billed to** band, and an optional **invoice number** (prefix + counter). ([brand refresh](https://github.com/PunchIn-App/punchin))
- **Time format & currency.** New settings let you choose **12- or 24-hour** clock times (timesheets + invoices) and a **default currency** — invoice and CSV amounts now format with the right symbol and separators for your currency instead of a hardcoded `$`. ([brand refresh](https://github.com/PunchIn-App/punchin))
