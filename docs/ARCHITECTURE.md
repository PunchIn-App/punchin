...
│   │   ├── syncManager.js      # Core sync logic: exportSnapshot (data + portable preferences), mergeSnapshot (reuses import dedup; `{applySettings}` gate), importSnapshot (public merge for transfer links, issue #77 — applies preferences, an explicit user import), runSync (pull→merge→push; seeds preferences into a FRESH install only, never overwriting an established device), disconnectSync
│   │   └── providers/
