...
├── worker/
│   ├── oauth.js            # Cloudflare Worker: handles GitHub OAuth code→token exchange and best-effort provider token-revoke on disconnect (POST /oauth/revoke — GitHub revokes this device's token via client-secret HTTP Basic, device-scoped via …/token not the account-wide …/grant, so the secret never reaches the browser; Google is proxied through the worker too, keeping oauth2.googleapis.com out of the browser CSP); renders on-demand exact-colour accent install icons at /icons/i/<hex>/* (issue #228, falls back to the nearest static palette swatch on render failure); falls through to static assets for all other routes. Wraps every response with CSP/hardening headers
...
│   ├── App.jsx             # Root: tab state, theme application, accent → CSS var + dynamic favicon, OAuth callback handling (reads window.location.hash on mount), first-run install nudge (after ≥2 opens, localStorage-gated), and the first-run import nudge (FirstRunImport, shown once when the install has no data yet)
│   ├── sync/
...
│   │   ├── syncManager.js      # Core sync logic: exportSnapshot (data + portable preferences), mergeSnapshot (reuses import dedup; `{applySettings}` gate), importSnapshot (public merge for transfer links, issue #77 — applies preferences, an explicit user import), runSync (pull→merge→push; seeds preferences into a FRESH install only, never overwriting an established device), disconnectSync (deletes the device gist file, then best-effort revokes this device's GitHub/Google token via the worker /oauth/revoke — device-scoped, so other devices keep syncing — OneDrive has no client-side revoke, relying on its prompt=select_account + ~1h expiry — then wipes the token + sync settings)
│   │   └── providers/
...
│   │       ├── google.js       # Google Drive API: buildGoogleOAuthUrl (implicit flow, appdata scope — stays implicit; see the #128 comment for why; prompt=select_account forces the account chooser on reconnect), pushToDrive, pullFromDrive
│   │       └── onedrive.js     # Microsoft Graph API: buildOneDriveOAuthUrl (Auth Code + PKCE, AppFolder scope, issue #128; prompt=select_account forces the account chooser on reconnect) + exchangeOneDriveCode (client-side token exchange, token never in URL), pushToOneDrive, pullFromOneDrive
