...
│   │   ├── LaborGlyph.jsx      # Labor-type glyphs (a Lucide icon beside the colour, so types read by shape + colour): LABOR_GLYPHS map + glyphComponent() (Tag fallback), LaborGlyphChip (solid colour chip + glyph, ink via inkOnAccent) and LaborTag (tinted pill + glyph + name). Used by the editor + every labor-type render site
