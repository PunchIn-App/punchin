...
│   │   ├── EntitySelect.jsx    # Bespoke single-select replacing native <select> where choices carry colour/glyph (jobs, labor types): listbox of options { value,label,sublabel?,color?,glyph? } showing a colour dot (jobs) or labor glyph (a `glyph` key — unset falls back to the PunchIn brand mark) + label + sublabel; configurable empty/clear row; capture-Escape + outside-click. Default in-flow menu (no clip in scroll-container modals); a `compact` variant renders a toolbar-chip trigger + floating menu (auto left/right-anchored to stay in the viewport) for the Timesheets filter row. Backs the Start Timer, Edit Entry, Invoice, and Job-form pickers + the Timesheets job/labor filters
